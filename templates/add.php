...
<form class="form form--add-lot container<?=count($errors) ? ' form--invalid' : '' ?>" action="add.php" method="post" novalidate enctype="multipart/form-data">
    ...
    <div class="form__container-two">
        <div class="form__item<?=in_array('lot-name', $errors) ? ' form__item--invalid' : '' ?>">
            ...
            <input id="lot-name" type="text" name="lot-name" placeholder="Введите наименование лота" value="<?=strip_tags($_POST['lot-name']) ?? '' ?>" required>
        ...
        <div class="form__item<?=in_array('category', $errors) ? ' form__item--invalid' : '' ?>">
            ...
            <select id="category" name="category" required>
                ...
                <option value="<?=$value['id']; ?>" <?=(isset($_POST['category']) && $_POST['category'] == $value['id']) ? 'selected' : '' ?>><?=$value['title']; ?></option>
                <?php endforeach; ?>
            </select>
    ...
    <div class="form__item form__item--wide<?=in_array('message', $errors) ? ' form__item--invalid' : '' ?>">
        ...
        <textarea id="message" name="message" placeholder="Напишите описание лота" required><?=strip_tags($_POST['message']) ?? '' ?></textarea>
    ...
    <div class="form__container-three">
        <div class="form__item form__item--small<?=in_array('lot-rate', $errors) ? ' form__item--invalid' : '' ?>">
            ...
            <input id="lot-rate" type="number" name="lot-rate" placeholder="0" value="<?=strip_tags($_POST['lot-rate']) ?? '' ?>" required>
        ...
        <div class="form__item form__item--small<?=in_array('lot-step', $errors) ? ' form__item--invalid' : '' ?>">
            ...
            <input id="lot-step" type="number" name="lot-step" placeholder="0" value="<?=strip_tags($_POST['lot-step']) ?? '' ?>" required>
        ...
        <div class="form__item<?=in_array('lot-date', $errors) ? ' form__item--invalid' : '' ?>">
            ...
            <input class="form__input-date" id="lot-date" type="text" name="lot-date" placeholder="20.05.2017" value="<?=strip_tags($_POST['lot-date']) ?? '' ?>" required>
